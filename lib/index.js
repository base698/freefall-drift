angular.module('simulation', []).controller('SimulationCtrl', function($scope) {
  ...
  */
  ...
  function getInitConfig(config) {
    //console.log(config.timeBetweenGroups);
    var numJumpers = config.numJumpers || 15;
    var groupSwitch = config.groupSwitch || 6;
    var groupSize = config.groupSize || 4;
    var openingAltitude = config.openingAltitude || 3200;
    var timeBetweenGroups = Number(config.timeBetweenGroups) || 8;
    var airplaneSpeed = (config.airplaneSpeed || 100) * ftPerSecond;
